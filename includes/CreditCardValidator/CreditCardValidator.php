 ...
 * @Michele Vomera added the expire date and Name validation
 */
class CreditCardValidator
{
    ...
    /**
     * Checks if the credit card number expire date is valid.
     * 
     * @param string $expdate
     * @return bool
     */
    public function isValidDate($expdate)
    {
        $month;
        $year;
    	$currentmonth;
    	...
    	$pattern = '(-)';
    	$newexpdate = preg_replace($pattern, '/', $expdate);
    
    	$splits = explode('/',$newexpdate);
    	if(count($splits)>1){
    	   $month = trim($splits[0]);
    	   $year = trim($splits[1]);
    	}
        
        if(is_null($month) || is_null($year) || !is_numeric($month) ||!is_numeric($year))
        {
        	//year or month are null or not numeric
        	return false;
        }
        ...
        if($month >= 1 && $month<=12 //month is valid
            && $year<=($currentyear+15) //year is valid
            && (($currentmonth <= $month && $year==$currentyear) //month is valid in the current year
            || ($year>$currentyear) //year is valid
            )
          )
        {
        	return true;
